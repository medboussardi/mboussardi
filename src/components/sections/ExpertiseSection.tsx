...
const iconMap: Record<string, React.ReactNode> = {
  ...
  workflow: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" /></svg>,
  ...
  link: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>,
};

export default function ExpertiseSection() {
  const t = useTranslations('expertise');
  const domains = t.raw('domains') as Array<{
    title: string;
    description: string;
    icon: string;
  }>;
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {domains.map((domain, i) => (
            <AnimatedSection key={i} delay={i * 0.08}>
              <div className="group p-8 rounded-xl border border-slate-100 bg-white hover:border-navy-200 hover:shadow-lg hover:shadow-navy-100/50 transition-all duration-300">
                <div className="w-12 h-12 rounded-xl bg-navy-50 text-navy-600 flex items-center justify-center mb-5 group-hover:bg-navy-900 group-hover:text-white transition-colors duration-300">
                  {iconMap[domain.icon] || iconMap.building}
                </div>
                <h3 className="text-lg font-bold text-navy-900 mb-3">{domain.title}</h3>
